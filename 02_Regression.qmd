...
Odds are, you've used linear regression to see how a few variables predict some other variable. If you want to really dive into these, consider taking a normal people statistics class.
...
A whole bunch of steps, mostly data wrangling, which is not the focus here.

### Note on CLR transformation

I do a centered log-ratio (CLR\_ transformation on the microbial data, which is necessary for "relative abundance" data typical of microbes. See this excelent paper for why I need to do this.
...
Per Gloor et al. 2017, your data are compositional if their total value is an arbitrary data imposed by the measurment instrument. In the case of microorganims data, my total values are "relative abundance". They sum to one because I told them too, which doesn't reflect the environment.

If I had somehow measured microbes not in % of total community but in cells/L (the focus of my current research), my data would not be compositional.

If your data are "compositional" you need to do this transformation, and if they are not, you don't.
...
# We shouldn't do stats on data that are bounded between zero and one for reasons. We need to centered log-ratio (CLR) transform the data, which we will discuss eventually. But for now, check out the steps.

# In the CLR transformation we 
...
# Reshaping -- see the data wrangling lesson
...
**Question:** Describe how this nested data frame is structured, in your own words. In particular, what is going on in the `data` column.

Now we can apply operations to each of the nested data frames
...
This dat frame has a bunch of coefficients for the slope and intercept of the relationship between each phylum and temperature.
We kind of don't care about the intercepts though, and the p values are hard to read as wrritten.

First, lets just keep the `temp`erature terms.
...
Ok, bad example. These actually look worse than the non-shuffled data, which is promising. We want things to look worse when we shuffle the data. If we shuffle 100 times, we can actually use that as a way of testing whether things look sane.
...
**Question:** What did I do to `random_data` to get the output above?
...
The false discovery rate (FDR) asks not how likely any one value is to be a false positive, but rather what the likely fraction of false positives likely is in your dataset. So if you have an FDR of 5%, of the things that make that cutoff, about 5% of them are likely to have emerged by random chance. Fortunately FDR is really easy to calculate.
...
phyla_temp_analysis_unnested_2 %>%
  ...
  mutate(fdr = p.adjust(p.value, method = "BH")) %>% # Just one line of code to calculate FDR values.
...
# I had this problem when I wrote the code.
```
...
*Task:* Make a new flextable where you italisize all of the false discovery rates that are smaller than 20% and format them as percentages, rather than numbers.
...
*Task:* Write a few sentences that might go in a results section describing what you know about which phyla are relted to temperature. Please include the words "(Table 1)".

# Many X

So you are likely more accustomed to the situation where you want to see how a single dependent variable, say `Gammaproteobacteria` abundance, relates to the abundance a bunch of independent variables, say enviornmental predictors. The `lm` function makes this pretty easy.
...
Lets combine the gammaproteobacteria data with the selected enviornmental data
...
Now I'm going to just take the rows where there are gamma-proteobacteria and at least one environmental column with data.
Then I'm going to impute the missing values with medians
```{r}
...
**Question:** How might the missing values be affecting this analysis?

# Putting it all together
We can of course do multi-x and multi-y regression with stuff we have so far.

First lets get rid of rows missing species data, and rows missing environenttal data
...
Pivot just the phyla longer, but not the environmental data
